...
if not API_KEY:
    raise RuntimeError("Faltou definir a variável de ambiente GEMINI_API_KEY no Render.")
...
# Alguns ambientes mudam nomes/disponibilidade. Vamos ter fallback:
MODEL_CANDIDATES = [
    "gemini-1.5-flash",      # preferido (rápido)
...
def get_model():
    ...
    for name in MODEL_CANDIDATES:
        try:
            ...
            last_err = e
    raise RuntimeError(f"Não consegui inicializar nenhum modelo. Último erro: {last_err}")
...
<body>
  ...
  <script>
    ...
    fileInput.onchange = (e) => {
      ...
      if (selectedFile) {
        chat.innerHTML += `<div class="msg u">📎 Foto anexada: ${selectedFile.name}</div>`;
...
def chat():
    try:
        ...
        if file_b64:
            # ✅ IMPORTANTE: o SDK espera BYTES (não string base64)
            img_bytes = base64.b64decode(file_b64)

            # Forma compatível do google-generativeai:
        ...
        if not text.strip():
            text = "Eu não consegui gerar uma resposta agora. Tente de novo."

        return jsonify({"reply": text})

    except Exception as e:
        # Melhor devolver o erro real (pra você debugar) sem “quebrar” a UI
        return jsonify({"reply": f"Erro técnico: {type(e).__name__}: {str(e)}"}), 200

# Render usa gunicorn, então não precisa app.run() em produção.
